<script setup lang="ts">
  ...
  const reroll_enabled = computed(() => game && game._rolls_left > 0 && enabled)
...
<template>
  <div class="dice">
    <div v-if="!enabled" class="diceheader">{{game.players[game._playerInTurn]}} is playing</div>
    <div class="die"></div>
    <div v-for="d in game._roll" :class="`die die${d}`">{{d}}</div>
    <div class="caption">{{(enabled && reroll_enabled)? 'Hold:' : ''}}</div>
    <input  v-if="enabled && reroll_enabled" v-for="(_, i) in game._roll" type="checkbox" v-model="held[i]"/>
