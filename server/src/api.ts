...
export default (broadcaster: Broadcaster, store: GameStore, randomizer: Randomizer) => {
  ...
  async function reroll(id: number, held: number[], player: string) {
    const game = await server.reroll(id, held, player);
    game.process(broadcast)
    return game
  }
  
  async function register(id: number, slot: SlotKey, player: string) {
  ...
  function pending_game(id: number) {
    return server.pending_game(id)
  }

  async function join(id: number, player: string) {
